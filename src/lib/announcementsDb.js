...
// 발행 — admin_publish_announcement RPC.
//   응답: { ok, id, published_at } | { ok:false, error }
//   ⚠️ p_actor 는 현재 로그인 운영자 UUID — 호출처에서 user.id 전달.
export async function publishAnnouncement({ title, body, isPinned, actorId }) {
  ...
  return { ok: true, id: data?.id, publishedAt: data?.published_at };
}
